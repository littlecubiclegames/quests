...
class QuestInitializerTest extends TestCase
{
    ...
    public function testInitialize()
    {
        ...
        $this->storage->expects($this->once())->method('getActiveQuests')->with($this->equalTo($userId))->willReturn([$quest1, $quest2]);
        $this->slotLoader->expects($this->once())->method('getSlotsForUser')->with($this->equalTo($userId))->willReturn([$slot1 => new Slot($slot1, 'registry'), $slot2 => new Slot($slot2, 'registry')]);
    ...
    public function testInitializeUnavailableSlot()
    {
        ...
        $this->slotLoader->expects($this->once())->method('getSlotsForUser')->with($this->equalTo($userId))->willReturn(['otherslot' => new Slot('otherslot', 'registry')]);
        $this->storage->expects($this->once())->method('getActiveQuests')->with($this->equalTo($userId))->willReturn([]);
