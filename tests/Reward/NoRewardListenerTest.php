...
class NoRewardListenerTest extends TestCase
{
    ...
    public function testHasNoReward()
    {
        ...
        $definition->expects($this->once())->method('hasReward')->willReturn(false);
        $this->registry->expects($this->once())->method('getQuest')->with($this->equalTo($questId))->willReturn($definition);
        $this->worfklow->expects($this->once())->method('can')->with($this->equalTo($quest), $this->equalTo(QuestDefinitionInterface::TRANSITION_COLLECT_REWARD))->willReturn(true);
        $this->worfklow->expects($this->once())->method('apply')->with($this->equalTo($quest), $this->equalTo(QuestDefinitionInterface::TRANSITION_COLLECT_REWARD));
    ...
    public function testHasReward()
    {
        ...
        $quest->expects($this->once())->method('getQuestId')->willReturn($questId);
        $definition = $this->getMockBuilder(Quest::class)->disableOriginalConstructor()->getMock();
        $definition->expects($this->once())->method('hasReward')->willReturn(true);
        $this->registry->expects($this->once())->method('getQuest')->with($this->equalTo($questId))->willReturn($definition);
        $this->worfklow->expects($this->never())->method('can');
        $this->worfklow->expects($this->never())->method('apply');
        $event = new Event($quest, new Marking(), new Transition('transition', '', ''));
        $this->listener->validate($event);
    }
}
